...
#include <stdio.h>
 ...
 bool read_cfh(struct cfh *cfh, const uint8_t *src, size_t src_len, size_t* offset) // Чтение одного центр каталога за раз
{
    const uint8_t *p;
    uint32_t signature;
    ...
    assert(p == &src[src_len - *offset + CFH_BASE_SZ] && "All fields read.");
